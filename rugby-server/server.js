// Chocoby rugby — authoritative Socket.io game server.
// Mirrors the local-prototype simulation in chocolet/rugby/app.js.
...
// =====================================================================
// Constants (must mirror the client's pitch / tunables)
// =====================================================================
const W = 1430;
const H = 780;
const TRY_L = 78;
...
const app = express();
app.use(cors());
app.get("/", (_req, res) => res.send("Chocoby rugby server is running."));
app.get("/health", (_req, res) =>
  res.json({
    ok: true,
    perTeam: room.perTeam,
    humans: room.players.filter((p) => p.socketId).length,
    bots: room.players.filter((p) => !p.socketId).length,
  })
);
...
const room = {
  perTeam: 3,
  players: [], // { team:'A'|'B', jersey, x, y, vx, vy, lastTackleMs, stunUntilMs, requestUntilMs, socketId, name, isBot }
  ...
  msg: "",
  msgUntilMs: 0,
  events: [], // queued visual events (impacts, shakes) since last broadcast
};

// Per-player input state, indexed by player array idx
// { dx, dy, aimX, aimY, mouseDown, prevMouseDown, chargeStartMs }
const inputs = [];
...
function setupKickoff(receivingTeam) {
  ...
  // Preserve existing socket bindings if present, otherwise create fresh.
...
function makeEmptyInput() {
  return {
    ...
    requestQueued: false,
    tackleQueued: false,
    passQueued: null, // { power, aimX, aimY }
  };
}
...
// =====================================================================
// Slot management — humans take bot slots, alternating teams
// =====================================================================
function findHumanSlot() {
  // Prefer the team with fewer humans, then first bot slot in that team.
  ...
  if (idx < 0) {
    // fall back to any bot slot
    idx = room.players.findIndex((p) => p.isBot);
  }
  return idx;
}
...
function tick() {
  ...
  // Player-player collisions (basic push-out)
  for (let i = 0; i < room.players.length; i++) {
  ...
  // Process queued human actions BEFORE ball physics so they take effect this tick
  if (!frozen) {
    for (let i = 0; i < room.players.length; i++) {
      const inp = inputs[i];
  ...
  if (room.ball.carrierIdx !== null) {
    ...
    if (room.ball.x < BALL_RADIUS) {
      room.ball.x = BALL_RADIUS;
      room.ball.vx = Math.abs(room.ball.vx) * 0.5;
    }
    if (room.ball.x > W - BALL_RADIUS) {
      room.ball.x = W - BALL_RADIUS;
      room.ball.vx = -Math.abs(room.ball.vx) * 0.5;
    }
    if (room.ball.y < BALL_RADIUS) {
      room.ball.y = BALL_RADIUS;
      room.ball.vy = Math.abs(room.ball.vy) * 0.5;
    }
    if (room.ball.y > H - BALL_RADIUS) {
      room.ball.y = H - BALL_RADIUS;
      room.ball.vy = -Math.abs(room.ball.vy) * 0.5;
    }
...
// =====================================================================
// State broadcast (compact snapshot)
// =====================================================================
function snapshot() {
  return {
    perTeam: room.perTeam,
...
io.on("connection", (socket) => {
  socket.on("join", (payload = {}) => {
    ...
    console.log(`[join] ${name} -> idx ${idx} (${p.team}#${p.jersey})`);
  });
...
httpServer.listen(PORT, () => {
  console.log(`Chocoby rugby server listening on :${PORT}`);
});
